...
describe('FullCitation', () => {
  it('renders citation text in h3', () => {
    const html = FullCitation({ citation: 'Genesis 1:1', displayOsisIDWork: false, osisIDWork: '', style: {} });
    expect(html).toContain('<h3');
    expect(html).toContain('Genesis 1:1');
  });

  it('includes OSIS ID when displayOsisIDWork is true', () => {
    const html = FullCitation({ citation: 'Genesis 1:1', displayOsisIDWork: true, osisIDWork: 'KJV', style: {} });
    expect(html).toContain('(KJV)');
  });

  it('omits OSIS ID when displayOsisIDWork is false', () => {
    const html = FullCitation({ citation: 'Genesis 1:1', displayOsisIDWork: false, osisIDWork: 'KJV', style: {} });
